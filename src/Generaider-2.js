...
function preload_generatore2 (s){
    img_generatore2 = PP.assets.image.load(s, "assets/immagini/porta1.png", 150, 156);
}

function create_generatore2 (s){
    generatore2 = PP.assets.image.add(s, img_generatore2, 3300, 1200, 0, 0);
...
function update_generatore2 (s) {
    if (generatore2.geometry.x <= 3300) {
        PP.physics.set_velocity_x(generatore2, 500);
    }
    if (generatore2.geometry.x >= 4000) {
        PP.physics.set_velocity_x(generatore2, -500);
    }
}
